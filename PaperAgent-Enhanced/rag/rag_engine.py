...
def get_embeddings():
    """根据配置返回 Embedding 实例。"""
    if config.EMBEDDING_SOURCE == 'fastembed':
        from rag.hybrid_retrieval import FastEncoder
        return FastEncoder(model_dir=config.FASTEMBED_MODEL_DIR)
...
class PaperVectorStore:
    ...
    def __init__(self, embeddings, index, chunks, metadata):
        ...
        from rag.hybrid_retrieval import HybridIndex
        records = [{"id": str(i), "chunk_id": str(i), "text": text,
                    "source": metadata[i].get("source", "unknown"),
                    "page": metadata[i].get("page")} for i, text in enumerate(chunks)]
        self.hybrid = HybridIndex(records, index.reconstruct_n(0, len(chunks)), embeddings)
    ...
    def from_texts(cls, texts, metadata, embeddings):
        ...
        if not texts or len(texts) != len(metadata):
            raise ValueError('texts and metadata must be nonempty and aligned')
    ...
    def search(self, query: str, top_k: int = None, mode=None, source=None) -> list[dict]:
        """
        检索最相似的文本块。
        v2: 返回结果包含 page 页码信息。
        """
        k = config.RERANKER_CANDIDATES if top_k is None else top_k
        selected_mode = mode or self.retrieval_mode
        # Same implementation as scripts/benchmark.py; keep existing reranker/LLM callers.
        return self.hybrid.search(query, k=k, pool=max(config.HYBRID_POOL,k),
                                  mode=selected_mode, weight=config.HYBRID_DENSE_WEIGHT, source=source)

    def search_faiss_baseline(self, query: str, top_k: int = 20) -> list[dict]:
        """Retained FAISS path for equivalence checks and larger indexes."""
        if top_k <= 0:
