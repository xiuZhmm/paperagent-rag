...
with st.sidebar:
    ...
    retrieval_mode = st.selectbox('召回方式', ['dense', 'bm25', 'hybrid'],
                                  index=['dense', 'bm25', 'hybrid'].index(config.RETRIEVAL_MODE))
    if st.session_state.vectorstore is not None:
        st.session_state.vectorstore.retrieval_mode = retrieval_mode
    st.markdown("---")
    ...
    upload_digest = hashlib.sha256(b''.join(f.name.encode()+b'\0'+f.getvalue() for f in uploaded_files)).hexdigest() if uploaded_files else None
    if uploaded_files and upload_digest != st.session_state.get('upload_digest'):
        os.makedirs(config.DATA_DIR, exist_ok=True)
        for f in uploaded_files:
            save_path = os.path.join(config.DATA_DIR, Path(f.name.replace('\\','/')).name)
    ...
    if pdf_count > 0 and not st.session_state.initialized:
        with st.spinner("正在处理论文，构建知识库..."):
            try:
                ...
                vs.retrieval_mode = retrieval_mode
